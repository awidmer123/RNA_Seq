...
The RNA-Seq data analysed in this project originate from a study by Singhania et al., which investigated immune responses to Toxoplasma gondii infection in mice with different genetic backgrounds. In that study, wild-type (WT) animals were compared to a double knockout (DKO) model lacking both the type I and type II interferon receptors (Ifnar⁻/⁻ and Ifngr⁻/⁻), enabling a systematic assessment of interferon-dependent transcriptional responses across multiple tissues. In the present practical course, RNA-Seq subsamples derived from this experimental setup were provided by the university and re-analysed with a specific focus on lung tissue. The aim of this project was to compare bulk transcriptional responses between infected (“case”) and uninfected (“control”) samples in WT and DKO mice, thereby assessing how the absence of both type I and type II interferon signalling affects infection-induced gene expression in the lung.
...
Principal component analysis (PCA) and gene expression heatmaps were generated using rlog - transformed counts. These visualisations revealed how samples cluster based on their global gene expression profiles and allowed the assessment of replicate consistency and separation of experimental groups.

Differential expression analysis was performed using DESeq2’s Wald test. Infection-induced transcriptional changes were first assessed within each genotype by comparing infected (“case”) and uninfected (“control”) samples for wild-type (WT) and double knockout (DKO) mice separately. In addition, direct genotype-dependent differences were evaluated by contrasting infected WT and infected DKO samples. Genes were considered differentially expressed at a false discovery rate of padj \< 0.05, and no additional log₂ fold-change threshold was applied. For each contrast, the total number of significant differentially expressed genes (DEGs) as well as the numbers of up- and down-regulated genes were recorded. Normalised counts of selected biologically relevant genes were further inspected to support interpretation of genotype-specific responses.

### 2.9 Gene Ontology enrichment analysis

Functional enrichment analysis was conducted using the clusterProfiler package, specifically the `enrichGO` function. The list of differentially expressed genes (Ensembl IDs) served as input, while the set of all detected genes formed the universe background. The *Mus musculus* annotation database `org.Mm.eg.db` provided the necessary GO mappings. The Biological Process (BP) ontology was examined, and resulting GO terms were summarised and visualised using standard clusterProfiler plotting functions. Interpretation focused on immune-related processes known to be involved in Toxoplasma infection and interferon signalling based on Singhania et al. (2019). Specifically, genes previously reported to be associated with interferon-driven immune responses were selected as a reference set and systematically tracked across contrasts. Their differential expression statistics were summarised in Table 2 to enable direct comparison between the current dataset and published signatures. The majority of tracked genes originated from modules L5 and L7, which were previously associated with interferon-driven inflammatory responses and innate immune activation during pulmonary Toxoplasma infection.

## 3. Results

### 3.1 Quality control and read alignment

Quality control of the raw sequencing data was performed using MultiQC. Assignment rates ranged from 64 to 76 % for 14 samples, while one sample exhibited a lower assignment rate of 59.9 %. Across samples, sequencing depth was high, with between \~30 and 47 million reads per library, and one sample reaching approximately 55 million reads. Overall sequence quality metrics were high and no major outlier samples were detected. However, Per Base Sequence Content analysis revealed pronounced nucleotide composition bias at the beginning of reads across all samples. Specifically, the first \~10–15 bases showed strong deviations from equal base representation, after which nucleotide frequencies stabilized along the remainder of the read length. This pattern was consistent across all libraries and is characteristic of RNA-seq data. FeatureCounts and general alignment statistics indicated consistently high assignment rates across samples, with the majority of reads being uniquely assigned to annotated genomic features. A moderate fraction of multimapping reads was observed, which is expected given shared sequence similarity among gene families and repetitive regions. No additional filtering based on alignment or multimapping statistics was applied for downstream differential expression analyses. Consequently, all samples were retained for downstream analyses. The full MultiQC report is available in the GitHub repository at `results/multiqc_report_1.html`.
...
#group information (sample names and treatments), was adjusted for right order
groups_info <- read.table("groups_info.txt")
names(groups_info) <- c("label", "condition")

#deleting information columns
counts_cut <- counts[, 6:ncol(counts)]

#trangforming matrix to dds format
counts_dds <- DESeqDataSetFromMatrix(
countData = counts_cut,
colData   = groups_info,
design    = ~ condition
)
#creating rlog and DESeq object
...
#-------------------------------------------------------------------------
#plot, table and color design
#-------------------------------------------------------------------------

#loading necessary packages for table and plot design
library(dplyr)
library(knitr)
library(kableExtra)

#panel theme for volcano plots
...
# make sure condition is a factor with stable level order
groups_info$condition <- factor(groups_info$condition)

# choose colors (example)
condition_cols <- c(
  "Lung_WT_Control"  = "#1b9e77",
  "Lung_WT_Case"     = "#d95f02",
  "Lung_DKO_Control" = "#7570b3",
  "Lung_DKO_Case"    = "#e7298a"
)
...
#-------------------------------------------------------------------------
#setup for pca plot using plotPCA() command on rlog transformed data
#-------------------------------------------------------------------------

pca_plot <- plotPCA(counts_dds_rlog, intgroup = "condition") +
  theme_bw() +
  labs(title = "PCA of rlog-transformed counts", x = "PC1", y = "PC2") +
  scale_color_manual(values = condition_cols, name = "condition") +
  theme(
...
#using assay() to get gene and samples matrix
...
# annotate colors for legend
ann_colors <- list(
  condition = condition_cols
)

#making heatmap with pheatmap() command
heatmap <- pheatmap(
  mat_top,
...
#-------------------------------------------------------------------------
#plot panel with pca and heatmap
#-------------------------------------------------------------------------

# using grid.newpage() and grid.draw() to avoid graphic overlap
pca_plot
grid::grid.newpage() 
...
#-------------------------------------------------------------------------
# data for differential gene analysis comparing different groups
#-------------------------------------------------------------------------
...
#    1. getting labels of tracked genes
#    2. creating plots
#-------------------------------------------------------------------------

# --- 1. getting labels for tracked genes ---

# Map ENSMUSG -> gene symbol
...
lab_WTco_vs_DKOco <- wt_dko_con_hits$gene_name

# --- 2. Build volcanoes with mapping + selectLab ---
# volcano plot WT control vs case
...
# volcano plot DKO control vs case
...
# volcano plot WT case vs DKO case
...
# volcano plot WT control vs DKO control
#This specific plot will not be shown in the final report. Interpretation of it does not support understanding of data in the view of the author.
Volcano_WTco_vs_DKOco <- EnhancedVolcano(
  WT_con_VS_DKO_con,
  lab = get_labels(WT_con_VS_DKO_con, ens2sym),
  x = "log2FoldChange",
  y = "padj",
  title = NULL,
  selectLab = lab_WTco_vs_DKOco,
  drawConnectors = TRUE,
  max.overlaps = Inf
) + panel_theme_volcano
```
...
#-------------------------------------------------------------------------
# setup for summary table, getting the numbers of differnetially expressed genes
#-------------------------------------------------------------------------

# DE genes (padj < 0.05)
#extracting numbers for WT
...
#| tbl-scap: ["Number of significantly differentially expressed genes"]

#-------------------------------------------------------------------------
# plotting summary table
#-------------------------------------------------------------------------

# using dplyr for nice layout
summary_DE |>
  dplyr::mutate(
...
#| fig-scap: ["Volcano plots illustrating differential gene expression"]
#| fig-subcap: ["WT Control vs Case", "DKO Control vs Case", "WT Case vs DKO Case", "WT Control vs DKO Control"]
#| fig-pos: "H"
#| fig-width: 12
#| fig-height: 8
#| out-width: "100%"
...
#-------------------------------------------------------------------------
#panel for volcano plot using a 2 column layout
#-------------------------------------------------------------------------
...
#| fig-scap: ["Gene Ontology enrichment"]
#| fig-subcap: ["WT Control vs Case", "DKO Control vs Case", "WT Case vs DKO Case", "WT Control vs DKO Control"]
#| fig-pos: "H"
#| fig-width: 12
#| fig-height: 8
...
#-------------------------------------------------------------------------
# enrichGO barplot panel for WT, DKO and WT case vs DKO case
#-------------------------------------------------------------------------
...
#| tbl-scap: ["Selected genes showing significant differential expression"]
...
Comparative analysis of gene expression further highlighted a pronounced genotype-dependent response to infection. In wild-type mice, infection induced strong and statistically significant upregulation of a broad set of immune-related genes. Many of these genes correspond to those previously described as part of immune response modules, particularly modules associated with type I interferon signaling (L5) and IFNγ/STAT1-mediated pathways (L7). Notably, several tracked genes—including Gbp2, Gbp5, and Stat2—were consistently overexpressed upon infection in wild-type animals. These genes are key components of interferon-driven host defense: guanylate-binding proteins such as GBP2 and GBP5 contribute to intracellular pathogen restriction, while STAT2 is an essential transcriptional mediator of type I interferon signaling. In contrast, the same genes were largely not differentially expressed in the double knockout condition, with only two of the tracked genes reaching statistical significance. This indicates a substantial loss of coordinated immune gene induction in the absence of intact interferon signaling.
...
Importantly, these results are in close agreement with previous transcriptomic analyses of Toxoplasma gondii infection, particularly those reported by Singhania et al., who identified robust interferon-driven immune modules as central determinants of the host response. The consistent overexpression of canonical interferon-stimulated genes observed in both studies supports the conclusion that interferon signaling constitutes a conserved and dominant axis of transcriptional immune activation in this infection model.
